import type { Player, Projectile, Fish, StunState } from '../shared/types';
...
export function draw(): void {
  ...
  for (let x = 0; x <= state.worldSize; x += 100) {
    ctx.moveTo(x, 0);
    ctx.lineTo(x, state.worldSize);
  }
  for (let y = 0; y <= state.worldSize; y += 100) {
    ctx.moveTo(0, y);
    ctx.lineTo(state.worldSize, y);
  }
  ctx.stroke();
...
export function drawProjectile(proj: Projectile): void {
  ctx.save();
...
export function drawFish(f: Fish): void {
  ctx.save();
...
export function drawMinimap(): void {
  ...
  const scale = minimapCanvas.width / state.worldSize;
...
export function drawCat(p: Player, stunnedState?: StunState): void {
  ctx.save();
