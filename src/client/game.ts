import { state, localPlayer, canvas, WORLD_SIZE } from './state';
...
export function update(): void {
  ...
  const isStunned = state.stunned[socketId]?.until > now;
  ...
  if (!isStunned) {
    ...
    if (dx !== 0 || dy !== 0) {
      ...
      localPlayer.x = Math.max(0, Math.min(WORLD_SIZE, localPlayer.x + dx * localPlayer.speed));
      localPlayer.y = Math.max(0, Math.min(WORLD_SIZE, localPlayer.y + dy * localPlayer.speed));
